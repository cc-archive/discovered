 ...
 * IndexingFilter.
 */
public class MappedRawFieldQueryFilter implements QueryFilter {
	...
	protected MappedRawFieldQueryFilter(String query_field, String index_field) {
		this(query_field, index_field, true);
	}

	/** Construct for the named field, lowercasing query values. */
	protected MappedRawFieldQueryFilter(String query_field, String index_field,
			float boost) {
		this(query_field, index_field, true, boost);
	}

	/** Construct for the named field, potentially lowercasing query values. */
	protected MappedRawFieldQueryFilter(String query_field, String index_field,
			boolean lowerCase) {
		this(query_field, index_field, lowerCase, 0.0f);
	}
	...
	public BooleanQuery filter(Query input, BooleanQuery output)
		...
		for (int i = 0; i < clauses.length; i++) {
			...
			TermQuery clause = new TermQuery(new Term(index_field, value));
	...
	public void setConf(Configuration conf) {
		this.conf = conf;
		setBoost(conf.getFloat("query.raw_mapped.boost", 1.0f));
	}
